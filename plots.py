...
def plot_bar_grouped(df, selected_columns, **kwargs):
    """[summary]
    ...
    ax.set_title(title)
    plt.tight_layout()
    plt.savefig(_FOLDER + filename)
    # plt.show()

def plot_heatmap(df, rows, columns, calc_field, aggfunc, **kwargs):
    """[summary]
...
def plot_heatmap_time(df, aggfunc, **kwargs):
